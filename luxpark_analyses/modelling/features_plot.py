...
FILE = args.file_path #"../results/wandb/DENOVO_GCNN_3p_uw_20231021_undersamping/cv-results/cv_relevantnodes.csv"
#FILE = "../results/wandb/DENOVO_GCNN_3p_uw_20231021_undersamping/cv-results/cv_relevantnodes.csv"
...
# remove non annotated metabolites
df = df[df['CHEMICAL_NAME'].notnull()]
df = df[df['SUB_PATHWAY'].notnull()]
#df = df.loc[~df['SUB_PATHWAY'].isin(["RNA Gene", "Pseudogene"])]
...
legend = plt.legend(handles=legend_labels, title='Pathway', loc='lower right',  framealpha=0.0,  markerfirst=False)
