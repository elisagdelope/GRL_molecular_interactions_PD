...
def display_graph_individual(adj_df, labels_dict=None, save_fig=False, path="./", name_file="graph.png", plot_title=None):
    ...
    if labels_dict is None:
        ...
        sm.set_array([])
        cbar = plt.colorbar(sm, ax=ax, orientation="horizontal", shrink=0.25, pad=0.01, ticks=[0, 1])
        cbar.set_label('Log-transformed normalized abundance',  fontsize=16)
        cbar.set_ticks([])
...
# plot largest connected component of relevant edges with labels for high degree nodes
    ...
    pos = nx.layout.kamada_kawai_layout(G_connected)
    ...
    if labels:
        ...
        if annotation is not None:
            node_labels = {node: annotation.loc[annotation['ANALYSIS_ID'] == node, 'CHEMICAL_NAME'].values[0] if node in selected_nodes else '' for node in G_connected.nodes}
        else:
...
def plot_largest_connected_components(G, k=0.2, labels=True, min_nodes=4, d=None, annotation=None, fig_save=True, path=".", plotname="cv_overlappingedges_components"):
    ...
    for i, component in enumerate(components):
        ...
        if labels:
            ...
            if annotation is not None:
                node_labels = {node: annotation.loc[annotation['ANALYSIS_ID'] == node, 'CHEMICAL_NAME'].values[0] if node in selected_nodes else '' for node in G_subgraph.nodes}
            else:
                node_labels = {node: node if node in selected_nodes else '' for node in G_subgraph.nodes}
            for node, label in node_labels.items():
                xy = pos[node]
                xytext = (xy[0] - 0.04, xy[1] + 0.02)  # Adjust the y-coordinate of the label
                plt.annotate(label, xy=xy, xytext=xytext, fontsize=14, fontname='Arial', bbox=dict(boxstyle="round,pad=0.3", alpha=0.2))
...
def plot_largest_connected_components2(G, labels=True, min_nodes=4, d=None, annotation=None, fig_save=True, path=".", plotname="cv_overlappingedges_components"):
    components = [comp for comp in nx.connected_components(G) if len(comp) >= min_nodes]
    # Generate a layout for the full graph
    ...
    for i, component in enumerate(components):
        ...
        if labels:
            ...
            for node, label in node_labels.items():
                xy = pos[node]
                xytext = (xy[0], xy[1] + 0.02)  # Adjust the y-coordinate of the label
                plt.annotate(label, xy=xy, xytext=xytext, fontsize=14, fontname='Arial', bbox=dict(boxstyle="round,pad=0.3", alpha=0.2))
        plt.tight_layout()  # Adjust layout for better spacing
        if fig_save:
    ...
    for i, G_subgraph in enumerate(all_subgraphs):
        ...
        if labels:
            ...
            for node, label in node_labels.items():
                xy = pos[node]
                xytext = (xy[0] - 0.03, xy[1] + 0.03)  # Adjust the y-coordinate of the label
                plt.annotate(label, xy=xy, xytext=xytext, fontsize=12, fontname='Arial', bbox=dict(boxstyle="round,pad=0.3", alpha=0.2))
    plt.tight_layout()
    if fig_save:
        # Save each subgraph separately
        subgraph_name = f"{plotname}_subgraph_{i + 1}"
...
def plot_subgraph_pathways(G_subgraph, annotation, labels=True, d=8, fig_save=True, i=0, path=".", plotname="cv_overlappingedges_components_pathway"):
    common_nodes = set(G_subgraph.nodes) & set(annotation['ANALYSIS_ID'])
    sub_pathways = annotation.loc[annotation['ANALYSIS_ID'].isin(common_nodes), 'SUB_PATHWAY'].fillna('Unknown')
    # pathways with >= 3 nodes in G_subgraph
    top_pathways = sub_pathways[sub_pathways.groupby(sub_pathways).transform('count') >= 3].unique()

    # Define color for each sub-pathway
    sub_pathway_colors = {}
    contrast_colors = ['#1f77b4', '#ff7a33', '#FFD700', '#FF33A1', '#17becf', '#008000', '#90a315', '#80FF33', '#d62728', '#7541fa', '#66FF33', '#ff8c00']
    j = 0
    for i, sub_pathway in enumerate(top_pathways):
        # Assign the same color for all sub-pathways starting with "Drug"
        if sub_pathway.startswith("Drug"):
            sub_pathway_colors[sub_pathway] = contrast_colors[0]
        else:
            j += 1
            sub_pathway_colors[sub_pathway] = contrast_colors[j]
    sub_pathway_colors['Other pathways'] = 'gray'

    # Define plot
    plt.figure(figsize=(14, 12))
    pos = nx.layout.kamada_kawai_layout(G_subgraph)

    # Draw edges (width based on edge attribute)
    edge_widths = [next(iter(data.values())) for _, _, data in G_subgraph.edges(data=True)] 
    nx.draw_networkx_edges(G_subgraph, pos=pos, edge_color='#cccccc', width=edge_widths) # color is light gray

    plotted_pathways = set()
    # Draw nodes with the same color as the pathway
    for subpathway, color in sub_pathway_colors.items():
        if subpathway == 'Other pathways':
            # For "Other pathways", get nodes not in the top pathways
            nodes_in_subpathway = [node for node in common_nodes if pd.notna(annotation.loc[annotation['ANALYSIS_ID'] == node, 'SUB_PATHWAY']).values[0] and annotation.loc[annotation['ANALYSIS_ID'] == node, 'SUB_PATHWAY'].values[0] not in top_pathways]
        else:
            nodes_in_subpathway = [node for node in common_nodes if pd.notna(annotation.loc[annotation['ANALYSIS_ID'] == node, 'SUB_PATHWAY']).values[0] and annotation.loc[annotation['ANALYSIS_ID'] == node, 'SUB_PATHWAY'].values[0] == subpathway]

        # Check if there are nodes for the current subpathway in the subgraph
        if len(nodes_in_subpathway) > 0:
            # Draw nodes with the same color as the pathway 
            nx.draw_networkx_nodes(G_subgraph, pos=nx.layout.kamada_kawai_layout(G_subgraph), nodelist=nodes_in_subpathway, node_size=100, node_color=[color] * len(nodes_in_subpathway), alpha=0.8)
            node_positions = [pos[node] for node in nodes_in_subpathway]

            # If > 1 node, create a convex hull
            if len(node_positions) > 1:
                hull = MultiPoint(node_positions).convex_hull
                if isinstance(hull, Polygon):
                    x, y = hull.exterior.xy
                elif isinstance(hull, LineString):
                    x, y = hull.xy
            else:
                # If only 1 node, create a small circle around it
                x, y = pos[nodes_in_subpathway[0]]
                circle = plt.Circle((x, y), 0.02, color=color, alpha=0.5)
                plt.gca().add_patch(circle)
                continue  # Skip drawing the patch for single nodes

            # Draw the convex hull/circle as a patch for subpathways =! Drug / 'Other pathways'
            if (subpathway != 'Other pathways') and (not subpathway.startswith("Drug")):
                pltpath = Path(list(zip(x, y)))
                patch = PathPatch(pltpath, facecolor=color, alpha=0.5, edgecolor='none')
                plt.gca().add_patch(patch)

                # Update the set of plotted pathways
                plotted_pathways.add(subpathway)
    ...
    # Show only the legend for plotted pathways
    legend_handles = [Patch(color=sub_pathway_colors[pathway], label=pathway) for pathway in plotted_pathways]
    legend_handles.append(Patch(color=sub_pathway_colors['Drug - Neurological'], label='Drug (Neurological, Analgesics/Anesthetics, Psychoactive, Antibiotic, Gastrointestinal)'))
    legend_handles.append(Patch(color=sub_pathway_colors['Other pathways'], label='Other pathways'))
    legend = plt.legend(handles=legend_handles, prop={'size': 13.5}, framealpha=0.2, edgecolor='none', loc='lower left', bbox_to_anchor=(0.07, 0.06))
