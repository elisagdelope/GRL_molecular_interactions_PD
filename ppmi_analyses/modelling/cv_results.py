...
DIR = args.dir_path #"../results/wandb/Cheb_GCNN_3p_20230930"
...
val_files = [file_name for file_name in val_files if int(re.search(r'\d+', file_name).group()) <= 130] # select first 130 sweeps in case there are more
...
nodes_df = nodes_df.merge(annotation, left_on="Feature", right_on="ensembl_gene_id", how="left")
nodes_df.drop("ensembl_gene_id", axis=1, inplace=True)
...
edges_df = edges_df.merge(annotation, left_on="source", right_on="ensembl_gene_id", how="left")
edges_df.drop(["ensembl_gene_id"], axis=1, inplace=True)
edges_df.rename(columns=dict(zip(['wikigene_name', 'wikigene_description'], [col + '_source' for col in ['wikigene_name', 'wikigene_description']])), inplace=True)
edges_df = edges_df.merge(annotation, left_on="dest", right_on="ensembl_gene_id", how="left")
edges_df.drop(["ensembl_gene_id"], axis=1, inplace=True)
edges_df.rename(columns=dict(zip(['wikigene_name', 'wikigene_description'], [col + '_dest' for col in ['wikigene_name', 'wikigene_description']])), inplace=True)
...
# plot largest connected component of relevant edges with labels for high degree nodes (>=d)
...
node_degree = node_degree.merge(annotation, left_on="Node", right_on="ensembl_gene_id", how="left")
node_degree.drop("ensembl_gene_id", axis=1, inplace=True)
...
node_counts = node_counts.merge(annotation, left_on="Node", right_on="ensembl_gene_id", how="left")
node_counts.drop("ensembl_gene_id", axis=1, inplace=True)
...
nodes_df.rename(columns= {'wikigene_name': 'Gene_symbol', 'wikigene_description': 'Annotation'}, inplace=True)
nodes_df.to_csv(DIR_RESULTS + "/cv_relevantnodes.csv", index=False)
node_degree.rename(columns= {'wikigene_name': 'Gene_symbol', 'wikigene_description': 'Annotation'}, inplace=True)
node_degree.to_csv(DIR_RESULTS + "/cv_relevantedges_nodesdegree.csv", index=False)
node_counts.rename(columns= {'wikigene_name': 'Gene_symbol', 'wikigene_description': 'Annotation'}, inplace=True)
